...
色彩以一组低饱和的深色中性层级为底；白色文字略带柔和感，青绿色只负责状态和细小强调。
...
`/resume` 是 592px 内容栏内的结构化在线履历，而不是下载页或宣传落地页。它从 `resume` content 读取并按介绍、联系胶囊、工作时间线、项目经验、三组技能和教育经历的顺序呈现；工作时间线按日期、公司名、职位纵向阅读，职位位于公司名下方并保持 8px 间距；技能卡片与教育经历使用墨黑胶囊同级的深色表面，并叠加适配深色画布的低对比细密网点，建立履历末段的共同材质；桌面时间线和技能可以分栏，390px 等手机宽度自然收为单列，联系入口保持 44px 最小高度。
